...
public class EstudianteController implements IBuscador {
    ...
    @Override
    public void buscarEsdutidantesPorCurso(String cursoNombre) {
        if (cursoNombre == null || cursoNombre.isEmpty() || cursoNombre.equals("Seleccionar...")) {
        ...
        for (Estudiante e : estudiantes) {
            if (e != null && e.getCurso() != null) {
                for (Curso c : e.getCurso()) {
                    if (String.valueOf(c.getCodigo()).equalsIgnoreCase(cursoNombre)) {
                        resultados.add(e);
                        break;
                    }
                }
